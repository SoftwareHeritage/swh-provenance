...
pub fn main() -> Result<()> {
    ...
    let reachable_nodes =
        swh_provenance_db_build::frontier_set::from_parquet(&graph, args.reachable_nodes, &mut pl)?;
    pl.done();
    ...
    <G as SwhGraphWithProperties>::Timestamps: swh_graph::properties::Timestamps,
{
    ...
    let on_revrel = |revrel: NodeId, path_parts: PathParts| {
        ...
        if !swh_provenance_db_build::filters::is_head(graph, revrel) {
            return Ok(());
        }
    ...
    swh_provenance_db_build::frontier::backward_dfs_with_path(
        graph,
        reachable_nodes,
        on_directory,
        on_revrel,
