...
def origin_add_revision(
    ...
    logging.debug(f'{"    "*depth}Adding revision {identifier_to_str(revision.swhid)} to origin {origin.id}')
...
def revision_walk_history(
    ...
    for parent in revision.parents:
        ...
        if not visited:
            ...
            logging.debug(f'{"    "*depth}Adding parent revision {identifier_to_str(parent)} to origin {origin.id}')
            cursor.execute('''INSERT INTO revision_in_org VALUES (%s,%s)
    ...
    for parent in storage.revision_get(to_rev):
        if parent is not None:
            parent = parent.to_dict()
            parent = RevisionEntry(parent['id'], parent['parents'])
            logging.debug(f'{"    "*depth}Adding parent revision {identifier_to_str(parent.swhid)} to revision {identifier_to_str(relative)}')
...
if __name__ == "__main__":
    ...
    for origin in OriginIterator(storage):
        ...
        for revision in origin.revisions:
            ...
            origin_add_revision(cursor, origin, revision, 0)
            comp_conn.commit()
        print(f'      ############################################################')
