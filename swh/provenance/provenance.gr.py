def revision_process_content(
    ...
    # but I put it here because I need to be sure, that all directories in the outer frontier
    # will be child of a directory (the root directory is a particular case)
            ...
            if (len(blobs)+len(dirs))==0:
                ...
                if maxdates<revision.date:
                    ...
                    if stack:
                        ...
                        if date==None:
                            # should the same as provenance.directory_get_early_date(dir)==None
                        ...
                        if date==None or date>maxdates: 
                            provenance.directory_set_date_in_isochrone_frontier(dir, maxdates) # ! make sure insert makes a min
                    ...
                    for child in dirs:
                        ...
                        if dateDir<revision.date:
                            ...
                            if provenance.directory_get_early_date(child)==None: 
                                # with this implementation you don't known if this is a new one or a reset
                                directory_process_content(provenance, directory=child, relative=child, prefix=PosixPath('.'))
                            provenance.directory_set_date_in_isochrone_frontier(child, cdate) # ! make sure insert = min
